...
const UserMarker = React.memo(function UserMarker({ user: u, onStartChat, t, getFaultTypeTranslation, roleColor, setSelectedObj, sos, currentUser, currentUserRole }: {
  user: any;
  onStartChat?: (userId: string, userName: string) => void;
  t: TFunction;
  getFaultTypeTranslation: (faultType: string | undefined) => string;
  ...
  const eventHandlers = useMemo(() => {
    if (!setSelectedObj) return undefined;
    return {
...
const ReportMarker = React.memo(function ReportMarker({ report, isSelected, t, setSelectedObj, onViewReportDetails }: {
  report: any;
  isSelected: boolean;
  t: TFunction;
  setSelectedObj?: (obj: any) => void;
  ...
  const eventHandlers = useMemo(() => {
    if (!setSelectedObj && !onViewReportDetails) return undefined;
    return {
      click: () => {
  ...
  }, [setSelectedObj, onViewReportDetails, report]);
...
const EventMarker = React.memo(function EventMarker({ event, setSelectedObj, onJoin, onViewEventDetails, isJoining, currentUser, t }: {
  event: any;
  setSelectedObj?: (obj: any) => void;
  ...
  const eventHandlers = useMemo(() => {
    if (!setSelectedObj && !onViewEventDetails) return undefined;
    return {
      click: () => {
  ...
  }, [setSelectedObj, onViewEventDetails, event]);
