...
import {
  startAppleSignIn,
  getAppleRedirectError,
  clearAppleRedirectError,
  isAppleRedirectPending,
  setAppleAuthError,
} from '../lib/appleAuth';
...
export function Auth({ initialIsLogin, onShowLanding }: AuthProps = {}) {
  ...
  React.useEffect(() => {
    const googleErr = getGoogleRedirectError();
    if (googleErr) {
      setError(googleErr);
      clearGoogleRedirectError();
    }
    const appleErr = getAppleRedirectError();
    if (appleErr) {
      setError(appleErr);
      clearAppleRedirectError();
    }
  ...
  const handleAppleSignIn = async () => {
    ...
    const intent = isLogin ? 'login' : 'signup';
    try {
      const appleUser = await startAppleSignIn(intent);
      if (!appleUser) return; // redirect in corso

      const resolvedUser = appleUser ?? (await waitForFirebaseUserAfterOAuth(8000));
      if (!resolvedUser) {
        setAppleAuthError('Accesso Apple non completato. Riprova.');
        setError('Accesso Apple non completato. Riprova.');
        return;
      }

      const resolution = await completeOAuthSession(resolvedUser, intent);
      if (resolution.status === 'needs_signup') {
        setIsLogin(false);
        setValue('name', resolvedUser.displayName || '');
        setValue('email', resolvedUser.email || '');
      ...
      if (authErr?.code === 'auth/unauthorized-domain') {
        ...
        setError('Apple Sign-In non ancora abilitato. Contatta il supporto.');
      } else {
        setError(authErr?.message || 'Errore durante il login con Apple');
      }
      setAppleAuthError(authErr?.message || 'Errore durante il login con Apple');
    } finally {
      if (!isAppleRedirectPending()) setLoading(false);
    }
  };
            ...
            <>
              ...
              <div className="flex gap-3">
                <button 
                  ...
                  disabled={loading}
                  className="flex-1 bg-white text-black shadow-sm border border-grey/10 font-bold py-4 rounded-xl hover:bg-grey/5 transition-all flex items-center justify-center gap-2"
                >
                ...
                >
                  ...
                  <svg className="w-5 h-5" viewBox="0 0 814 1000" fill="currentColor">
                    <path d="M788.1 340.9c-5.8 4.5-108.2 62.2-108.2 190.5 0 148.4 130.3 200.9 134.2 202.2-.6 3.2-20.7 71.9-68.7 141.9-42.8 61.6-87.5 123.1-155.5 123.1s-85.5-39.5-164-39.5c-76.5 0-103.7 40.8-165.9 40.8s-105-57.8-155.5-127.4C46 790.7 0 663 0 541.8c0-207.5 135.4-317.3 267.1-317.3 72.2 0 131.9 43.7 177.4 43.7 43.1 0 111.1-46 191.5-46 30.4 0 108.2 2.6 168.7 75.4zm-156.5-103.5c28.4-33.7 47.9-80.6 47.9-127.4 0-6.5-.6-13-1.9-19.5-45.3 1.9-99.7 30.4-131.8 68.1-26 29.1-48.5 75.4-48.5 122.9 0 7.1 1.3 14.3 1.9 16.5 2.6.6 6.5 1.3 10.4 1.3 40.9 0 91.4-27.2 121.9-61.9z"/>
                  </svg>
                  Apple
                </button>
              </div>
